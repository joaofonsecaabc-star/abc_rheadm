...
export const onRequest = async ({ request, env }: { request: Request; env: any }) => {
  ...
  if (request.method === 'GET') {
    const result = await env.DB.prepare(`SELECT id, username, full_name AS fullName, role, active, modules, store_access AS storeAccess,
  ...
  try {
    if (request.method === 'POST') {
      ...
      await env.DB.prepare(`INSERT INTO users(username,full_name,password_hash,password_salt,role,active,modules,store_access)
        VALUES (?,?,?,?,?,1,?,?)`).bind(username, fullName, hash, salt, role, modulesValue, storeAccess).run()
    ...
    if (request.method === 'PUT') {
      ...
      const statements = [env.DB.prepare(`UPDATE users SET username=?, full_name=?, role=?, active=?, modules=?, store_access=?, updated_at=CURRENT_TIMESTAMP WHERE id=?`)
        .bind(username, fullName, role, body.active === false ? 0 : 1, modulesValue, storeAccess, id)]
