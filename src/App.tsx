...
function AdministrativePage({ page, employees, companies, companyCnpjs, financialEntries }: { page: string; employees: Recharge[]; companies: string[]; companyCnpjs: Record<string, string>; financialEntries: FinancialEntry[] }) {
  ...
  return (
    <main className="p-4 sm:p-7">
      ...
      {(page === "Salário" || page === "Adiantamento") && <section className="mb-6 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl shadow-slate-200/50">
        <div className="bg-gradient-to-r from-slate-950 via-slate-900 to-slate-800 px-5 py-6 text-white sm:px-8">
          <div className="flex items-center gap-3">
            <span className="grid h-12 w-12 place-items-center rounded-2xl bg-white/10 text-white ring-1 ring-white/20"><ReceiptText size={23} /></span>
            <div><span className="text-xs font-bold uppercase tracking-[0.2em] text-slate-400">Documento administrativo</span><h2 className="mt-1 text-xl font-black">{page === "Salário" ? "Recibo de salário" : "Recibo de adiantamento"}</h2><p className="mt-1 text-sm text-slate-300">{page === "Salário" ? "Informe o período e acompanhe o cálculo proporcional em tempo real." : "Preencha os dados para gerar o recibo do adiantamento salarial."}</p></div>
          </div>
        </div>
        <div className="grid gap-5 bg-slate-50/80 p-5 sm:grid-cols-2 sm:p-8">
          <div className="sm:col-span-2 grid gap-3 sm:grid-cols-3">
            <div className={`flex items-center gap-3 rounded-2xl border p-4 transition ${receiptPerson ? "border-emerald-200 bg-emerald-50" : "border-slate-200 bg-white"}`}><span className={`grid h-9 w-9 place-items-center rounded-full ${receiptPerson ? "bg-emerald-600 text-white" : "bg-slate-100 text-slate-500"}`}>{receiptPerson ? <Check size={18} /> : <UserRound size={18} />}</span><div><b className="block text-sm">1. Identificação</b><span className="text-xs text-slate-500">Funcionário e empresa</span></div></div>
            <div className={`flex items-center gap-3 rounded-2xl border p-4 transition ${(receiptKind === "advance" ? receiptPeriod : salaryPeriodDays > 0) ? "border-blue-200 bg-blue-50" : "border-slate-200 bg-white"}`}><span className={`grid h-9 w-9 place-items-center rounded-full ${(receiptKind === "advance" ? receiptPeriod : salaryPeriodDays > 0) ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-500"}`}><CalendarDays size={18} /></span><div><b className="block text-sm">2. Período</b><span className="text-xs text-slate-500">Competência e datas</span></div></div>
            <div className={`flex items-center gap-3 rounded-2xl border p-4 transition ${(receiptKind === "salary" ? proportionalSalary > 0 : parseMoney(receiptAdvance) > 0) ? "border-violet-200 bg-violet-50" : "border-slate-200 bg-white"}`}><span className={`grid h-9 w-9 place-items-center rounded-full ${(receiptKind === "salary" ? proportionalSalary > 0 : parseMoney(receiptAdvance) > 0) ? "bg-violet-600 text-white" : "bg-slate-100 text-slate-500"}`}><Banknote size={18} /></span><div><b className="block text-sm">3. Valores</b><span className="text-xs text-slate-500">Cálculo e descontos</span></div></div>
          </div>
          <div className="sm:col-span-2 grid grid-cols-2 rounded-2xl border border-slate-200 bg-slate-200/70 p-1.5">
            <button type="button" onClick={() => setReceiptPersonMode("registered")} className={`rounded-xl px-4 py-3 text-sm font-bold transition ${receiptPersonMode === "registered" ? "bg-white text-slate-950 shadow-md" : "text-slate-500 hover:text-slate-800"}`}>Funcionário cadastrado</button>
            <button type="button" onClick={() => setReceiptPersonMode("manual")} className={`rounded-xl px-4 py-3 text-sm font-bold transition ${receiptPersonMode === "manual" ? "bg-white text-slate-950 shadow-md" : "text-slate-500 hover:text-slate-800"}`}>Preencher manualmente</button>
          </div>
          {receiptPersonMode === "registered" ? (
            <label className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:col-span-2"><span className="mb-2 block text-xs font-black uppercase tracking-wide text-slate-500">Funcionário</span><select value={receiptEmployeeId} onChange={(event) => setReceiptEmployeeId(event.target.value)} className="h-12 w-full rounded-xl border border-slate-200 px-4 font-semibold"><option value="">Selecione o funcionário</option>{activeEmployees.map((employee) => <option key={employee.id} value={employee.id}>{employee.employee}</option>)}</select></label>
          ) : (
            <>
          ...
          <label className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><span className="mb-2 block text-xs font-black uppercase tracking-wide text-slate-500">Loja / empresa</span><select value={receiptCompany} onChange={(event) => setReceiptCompany(event.target.value)} className="h-12 w-full rounded-xl border border-slate-200 px-4 font-semibold"><option value="">Selecione a loja</option>{companies.map((company) => <option key={company} value={company}>{company}{companyCnpjs[company] ? ` - ${companyCnpjs[company]}` : ""}</option>)}</select>{!companies.length && <span className="mt-1 block text-xs text-amber-600">Cadastre a loja e o CNPJ em Configurações → Lojas e empresas.</span>}</label>
          <label className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><span className="mb-2 block text-xs font-black uppercase tracking-wide text-slate-500">Competência</span><input type="month" value={receiptPeriod} onChange={(event) => setReceiptPeriod(event.target.value)} className="h-12 w-full rounded-xl border border-slate-200 px-4 font-semibold" /></label>
          <label className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><span className="mb-2 block text-xs font-black uppercase tracking-wide text-slate-500">Data do recibo</span><input type="date" value={receiptDate} onChange={(event) => setReceiptDate(event.target.value)} className="h-12 w-full rounded-xl border border-slate-200 px-4 font-semibold" /></label>
          {receiptKind === "salary" && <>
            <label className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><span className="mb-2 block text-xs font-black uppercase tracking-wide text-slate-500">Início do período trabalhado</span><input type="date" value={receiptSalaryStart} onChange={(event) => setReceiptSalaryStart(event.target.value)} className="h-12 w-full rounded-xl border border-slate-200 px-4 font-semibold" /></label>
            <label className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><span className="mb-2 block text-xs font-black uppercase tracking-wide text-slate-500">Fim do período trabalhado</span><input type="date" min={receiptSalaryStart} value={receiptSalaryEnd} onChange={(event) => setReceiptSalaryEnd(event.target.value)} className="h-12 w-full rounded-xl border border-slate-200 px-4 font-semibold" /></label>
            <label className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:col-span-2"><span className="mb-2 block text-xs font-black uppercase tracking-wide text-slate-500">Salário bruto mensal</span><input inputMode="decimal" value={receiptGross} onChange={(event) => setReceiptGross(formatMoneyInput(event.target.value))} placeholder="0,00" className="h-14 w-full rounded-xl border border-slate-200 px-4 text-lg font-black" /></label>
