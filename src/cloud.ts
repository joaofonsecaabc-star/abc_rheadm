...
export async function listCloudUsers(){if(!cloudEnabled())return [];return (await apiJson<{users:Array<Omit<CloudUser,'modules'>&{modules:unknown}>}>('/api/users')).users.map(user=>({...user,active:Boolean(user.active),modules:normalizeModules(user.modules),storeAccess:user.storeAccess||'*'}))}
...
export async function cloudCurrentUser():Promise<SessionUser|null>{
  ...
  const data=await response.json() as {user?:{id:number;username:string;fullName?:string;full_name?:string;role:'admin'|'operator';modules?:unknown;storeAccess?:string;store_access?:string}}
  if(!data.user)return null
  return {id:data.user.id,username:data.user.username,fullName:data.user.fullName||data.user.full_name||data.user.username,role:data.user.role,modules:normalizeModules(data.user.modules),storeAccess:data.user.storeAccess||data.user.store_access||'*'}
}
