...
export default function HRReports({employees,occurrences}:{employees:Recharge[];occurrences:Occurrence[]}){
  ...
  const results=useMemo<ReportResult[]>(()=>{
    ...
    const build=(type:ReportType):ReportResult=>{
      ...
      if(type==="Cumprindo aviso"){const start=new Date(`${month}-01T00:00:00`),end=new Date(start.getFullYear(),start.getMonth()+1,0,23,59,59),list=active.filter(employee=>employee.noticeStart&&employee.noticeEnd&&new Date(employee.noticeStart+"T12:00:00")<=end&&new Date(employee.noticeEnd+"T12:00:00")>=start);return {type,headers:["Funcionário","CPF","Cargo / setor","Loja","Início do aviso","Término do aviso","Dias restantes"],rows:list.map(employee=>{const finish=new Date(employee.noticeEnd+"T12:00:00"),days=Math.ceil((finish.getTime()-now.getTime())/86400000);return {values:[employee.employee,employee.cpf||"-",employee.role,employee.store,format(employee.noticeStart),format(employee.noticeEnd),days>=0?days:"Encerrado"]}})}}
